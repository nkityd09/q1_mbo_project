...
def create_nsg_rule_knox(rg_name, nsg_name, vnet_cidr):
    ...
    internal_comm_icmp = sp.getoutput(f""" az network nsg rule create --resource-group {rg_name} --nsg-name {nsg_name} --name InternalCommunicationICMP --priority 104 --source-address-prefixes {vnet_cidr} --protocol Icmp """)
...
def create_nsg_rule_default(rg_name, nsg_name, vnet_cidr):
    ...
    experiences = sp.getoutput(f""" az network nsg rule create --resource-group {rg_name} --nsg-name {nsg_name} --name Experiences --priority 902 --source-address-prefixes {vnet_cidr} --source-port-ranges '*' --destination-address-prefixes '*' --destination-port-ranges 443 """)
    ...
    internal_comm_icmp = sp.getoutput(f""" az network nsg rule create --resource-group {rg_name} --nsg-name {nsg_name} --name InternalCommunicationICMP --priority 105 --source-address-prefixes {vnet_cidr} --protocol Icmp """)
